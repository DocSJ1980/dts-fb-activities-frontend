...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("Outdoor surveillance API error:", error);
    return NextResponse.json(
...
async function handleGetSurveillanceData(searchParams: URLSearchParams) {
  try {
    ...
    console.log("API: Getting outdoor surveillance data with filters:", filters);

    const surveillanceData = await getOutdoorSurveillanceData(filters);

    console.log("API: Returning outdoor surveillance data:", {
    ...
    console.error("Error fetching outdoor surveillance data:", error);
    return NextResponse.json(
      { error: "Failed to fetch outdoor surveillance data" },
      { status: 500 }
    );
  }
}